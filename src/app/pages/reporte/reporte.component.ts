...
export class ReporteComponent implements OnInit {
  ...
  convertir(data: any){//de blod a base64
  ...
  dibujar() {
   ...
   generarReporte(){
    this.consultaService.generarReporte().subscribe(data => {
      ...
      reader.onload = (e: any) => {
        this.pdfSrc = e.target.result;//libreria externa
  ...
  descargarReporte(){
    this.consultaService.generarReporte().subscribe(data => {
      ...
      console.log(url);
      const a = document.createElement('a');//creando un atributo a
